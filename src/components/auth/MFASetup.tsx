...
export function MFASetup() {
  ...
  const [pendingRemoveFactorId, setPendingRemoveFactorId] = useState<string | null>(null);
  ...
  const performDisableMFA = async (fId: string) => {
    setLoading(true);
    try {
  ...
  const handleDisableMFA = async (fId: string) => {
    const { needsStepUp, factorId: stepFactor } = await checkAAL2StepUp();
    if (needsStepUp && stepFactor) {
      setPendingRemoveFactorId(fId);
      setStepUpFactorId(stepFactor);
      setStepUpOpen(true);
      return;
    }
    await performDisableMFA(fId);
  };
  ...
  return (
    <>
      ...
      {stepUpFactorId && pendingRemoveFactorId && (
        <MFAStepUpDialog
          open={stepUpOpen}
          onOpenChange={setStepUpOpen}
          factorId={stepUpFactorId}
          title="Confirm your identity"
          description="Enter your authenticator code to remove MFA from your account."
          onVerified={() => performDisableMFA(pendingRemoveFactorId)}
        />
      )}
    </>
  );
