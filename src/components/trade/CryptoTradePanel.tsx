...
export function CryptoTradePanel({ symbol, cryptoAssetId, currentPrice }: CryptoTradePanelProps) {
  const { user } = useAuth();
    ...
    <Card className="border-0 bg-card/50 backdrop-blur-sm">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-medium">Crypto Trade</CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="p-3 rounded-lg bg-muted/30 space-y-2 text-sm">
          <div className="flex justify-between">
            ...
            <span className="font-mono font-semibold">${estimatedCost.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
          <div className="flex justify-between">
        ...
        >
          {placeOrder.isPending ? 'Placing Order...' : `${orderSide === 'buy' ? 'Buy' : 'Sell'} ${qty} ${symbol.split('/')[0]}`}
        </Button>
        
        <p className="text-xs text-muted-foreground text-center">
          Crypto trading is available 24/7. Prices are volatile.
        </p>
      </CardContent>
    </Card>
  );
