...
serve(async (req) => {
  ...
  try {
    ...
    const { ticker, start_date, end_date, timespan = 'day' } = await req.json();
    ...
    if (!POLYGON_API_KEY) {
      ...
      const simulatedData = generateSimulatedData(ticker, start_date, end_date);
      return new Response(JSON.stringify({ 
        data: simulatedData,
        simulated: true 
      }), {
    ...
    const polygonUrl = `https://api.polygon.io/v2/aggs/ticker/${ticker}/range/1/${timespan}/${start_date}/${end_date}?adjusted=true&sort=asc&apiKey=${POLYGON_API_KEY}`;
    ...
    const marketData = polygonData.results.map((bar: any) => ({
      timestamp: bar.t,
...
function generateSimulatedData(ticker: string, startDate: string, endDate: string) {
